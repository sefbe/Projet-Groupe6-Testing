...
from models import Vehicle  # Assure-toi que ce modèle existe avec les bons champs
...
def search_vehicle_by_registration(registrationNumber):
    vehicle = Vehicle.query.filter_by(registrationNumber=registrationNumber).first()
    if not vehicle:
...
def get_vehicles_by_max_price(maxPrice):
    vehicles = Vehicle.query.filter(Vehicle.rentalPrice <= maxPrice).all()
    return jsonify([v.to_dict() for v in vehicles]), 200
